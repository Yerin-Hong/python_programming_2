...
# 추가: Enemy Sprite 클래스(빨간 박스 위치에 적 배치)
class Enemy(pygame.sprite.Sprite):
    ...
    def update(self):   # 좌우로 살짝 움직이게 해서 "살아있는(움직이는) 적" 느낌을 줌.
...
# 추가: 코인(초록 원)영 Rect - Rect 기반 충돌 예제용
coin_rect = pygame.Rect(430, 130, 40, 40)    # (450, 150) 근처, 원을 감싸는 사각형
score = 0    # 코인 먹은 횟수(게임의 느낌을 줌)
...
while running:
    ...
    if not game_over:
        all_sprites.update()   # 추가: game_over == False일 때만 업데이트

        # 1) Rect 기반 충돌: 플레이어 vs. 코인
        if player.rect.colliderect(coin_rect):
            print("코인 충돌")   # 코인 먹는 것도 충돌의 일종. 코인과 충돌하면,
            score += 1           # 스코어 증가.(1점씸)
    ...
    all_sprites.draw(screen)  # 추가: 스프라이트 그룹 업데이트(Player.update() 자동 호출)
