...
def borrow_book():
    ...
    if title not in books:
        books[title] = Book(title, author)      # 제목 기준 저장
...
def return_book():
    title = entry_title.get()    # 제목 입력창 값 가져오기
    
    if title == "":
        result_label.config(text="제목을 입력해주세요.", fg="red")
        return
    ...
    if title in books:
        ...
        result_label.config(text=f"{title}은 등록되지 않은 도서입니다.", fg="red")
...
button_borrow.grid(row=3, column=0, padx=10, pady=10)

button_return = Button(button_frame, text="반납", command=return_book, width=8)
button_return.grid(row=3, column=1, padx=10, pady=10)
